describe('Test pagination', () => {
      ...
      function LengthPagination(PageSize) {
          var length = Math.ceil(templateTaskList.length / PageSize)
          return length;
      }
      ...
      it('check number of display', () => {
          cy.get('.ant-select-selector').eq(2).as('NumberOfDisplay')
          ...
          if (arr[randIndex] <= templateTaskList.length) {
              ...
              cy.get('@' + arr[randIndex]).click()
              cy.get('[rel="nofollow"]').should('have.length', LengthPagination(arr[randIndex]))
              cy.get('.ant-table-row ').should('have.length', templateTaskList.length)
          }
      })
